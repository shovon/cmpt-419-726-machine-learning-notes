...
\newthought{I have no clue} why the following was in my notebook. Regardless,
I have typed out the following, in order to avoid the risk of losing any of it,
in case my notebook burns up somehow.

So let's say we have two events, $a$ and $b$, such that it has the following
...
replace $\sum\limits_{i = 1}^Nx_i$ for an unknown t, and finally isolate $\mu$.
